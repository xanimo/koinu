/* koinu.dog - base58 and base58check tests
...
{
    char out[128];
    size_t r = kw_base58_encode(in, n, out, sizeof out);
...
int main(void)
{
    ...
    /* raw base58 */
    n = (int)kw_test_unhex("61", b);            enc(b, n, "2g", "b58(61)");
    n = (int)kw_test_unhex("626262", b);        enc(b, n, "a3gV", "b58(626262)");
    n = (int)kw_test_unhex("636363", b);        enc(b, n, "aPEr", "b58(636363)");
    n = (int)kw_test_unhex("73696d706c792061206c6f6e6720737472696e67", b);
    ...
    /* raw round-trip, including a leading-zero byte -> leading '1' */
    {
        ...
        size_t r = kw_base58_encode(in, sizeof in, s, sizeof s);
        if (!r || s[0] != '1') { fprintf(stderr, "FAIL: leading zero not '1'\n"); fails++; }
        if (!kw_base58_decode(s, back, sizeof back, &bl) || bl != 5 || memcmp(in, back, 5)) {
    ...
    /* base58check address vectors: 0x00 || hash160 */
    {
        ...
        for (int i = 0; i < 2; i++) {
            uint8_t pay[21]; kw_test_unhex(v[i].hex, pay);
            char out[64];
            size_t r = kw_base58check_encode(pay, sizeof pay, out, sizeof out);
            ...
            if (!kw_base58check_decode(v[i].addr, back, sizeof back, &bl) ||
        ...
        if (kw_base58check_decode("1PMycacnJaSqwwJqjawXBErnLsZ7RkXUAt", junk, sizeof junk, &jl)) {
