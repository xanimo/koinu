...
int main(void)
{
    ...
    if (!dw_argon2id((const uint8_t *)"password", 8, (const uint8_t *)"somesalt", 8,
                     2, 65536, 1, out, sizeof out)) {
        fprintf(stderr, "FAIL: argon2id returned error\n"); return 1;
    }
    dw_test_check("argon2id t=2 m=65536 p=1", out, 32,
        "09316115d5cf24ed5a15a31a3ba326e5cf32edc24702987c02b6566f61913cf7");

    /* determinism, and that the cost parameters actually change the output */
    uint8_t again[32], other[32];
    dw_argon2id((const uint8_t *)"password", 8, (const uint8_t *)"somesalt", 8, 2, 65536, 1, again, 32);
    if (memcmp(out, again, 32) != 0) { fprintf(stderr, "FAIL: not deterministic\n"); return 1; }
    dw_argon2id((const uint8_t *)"password", 8, (const uint8_t *)"somesalt", 8, 3, 65536, 1, other, 32);
    if (memcmp(out, other, 32) == 0) { fprintf(stderr, "FAIL: t_cost had no effect\n"); return 1; }

    if (dw_test_fails()) { fprintf(stderr, "%d argon2 case(s) failed\n", dw_test_fails()); return 1; }
