...
int main(void)
{
    ...
    size_t n = dw_msg_serialize(DW_DOGE_MAINNET.magic, "verack", NULL, 0, out, sizeof out);
    if (n != 24) { fprintf(stderr, "FAIL: verack length %zu\n", n); return 1; }
    dw_test_check("verack frame", out, 24,
        "c0c0c0c076657261636b00000000000000000000" "5df6e0e2");

    /* parse it back */
    char cmd[13]; const uint8_t *pl = NULL; size_t plen = 0;
    int c = dw_msg_parse(DW_DOGE_MAINNET.magic, out, n, cmd, &pl, &plen);
    ...
    n = dw_msg_serialize(DW_DOGE_MAINNET.magic, "ping", nonce, sizeof nonce, out, sizeof out);
    c = dw_msg_parse(DW_DOGE_MAINNET.magic, out, n, cmd, &pl, &plen);
    if (c != (int)n || strcmp(cmd, "ping") != 0 || plen != 8 || memcmp(pl, nonce, 8) != 0) {
        fprintf(stderr, "FAIL: ping round trip\n"); return 1;
    }

    /* wrong magic is rejected */
    if (dw_msg_parse(DW_DOGE_TESTNET.magic, out, n, cmd, &pl, &plen) != -1) {
        fprintf(stderr, "FAIL: wrong magic accepted\n"); return 1;
    }

    /* a corrupted checksum is rejected */
    { uint8_t t[64]; memcpy(t, out, n); t[20] ^= 1;
      if (dw_msg_parse(DW_DOGE_MAINNET.magic, t, n, cmd, &pl, &plen) != -1) {
          fprintf(stderr, "FAIL: bad checksum accepted\n"); return 1; } }

    /* a header-only prefix asks for more, not an error */
    if (dw_msg_parse(DW_DOGE_MAINNET.magic, out, 23, cmd, &pl, &plen) != 0) {
    ...
    if (dw_msg_parse(DW_DOGE_MAINNET.magic, out, DW_MSG_HDR, cmd, &pl, &plen) != 0) {
